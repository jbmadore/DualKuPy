...
def main():
    ...
    if ok1 and ok2:
        ...
        fig, ax, lines, dashlines = init_plot(
            ("13GHz", "17GHz"), two_radar=True)
    ...
    keyboard_thread = threading.Thread(
        ...
        args=(cmd1, cmd2, com1, com2, ax, dashlines))
    ...
    while True:
        if running:
            if com1 and com2:
                ...
                rx_values1_copol = (
                    [abs(item) for item in data1['data'][0]][0:100])
                rx_values1_crosspol = (
                    [abs(item) for item in data1['data'][1]][0:100])
                rx_values2_copol = (
                    [abs(item) for item in data2['data'][0]][0:100])
                rx_values2_crosspol = (
                    [abs(item) for item in data2['data'][1]][0:100])
    ...
    while True:
        if com1 and com2:
            ...
            if key == "":
                ...
                if key == "1":
                    take_measurement(
                        cmd1, "Radar 13GHz", ax, dashlines, '13GHz')
                elif key == "2":
                    take_measurement(
                        cmd2, "Radar 17GHz", ax, dashlines, '17GHz')
                elif key == "b":
                    take_measurement(
                        (cmd1, cmd2), "Radar 13GHz and 17GHz",
                        ax, dashlines, 'both')
        elif com1 and not com2:
            key = input(
                "\nPress Enter to toggle display, or while pausing:\n"
...
def take_measurement(cmd, radar_label, ax, dashlines, measure_type=None):
    ...
    with recording_lock:
        ...
        print(f"Starting measurement for {radar_label} at site "
              f"{recording_info['site_name']} with angle "
              f"{recording_info['radar_angle']} and polarization "
              f"{recording_info['polarization']}...")
        ...
        if measure_type == 'both':
            ...
            plot_update_queue.put((
                "update_record", ax, dashlines, rx_values1_copol,
                rx_values1_crosspol, True, 'both',
            ...
            record_measurement(
                num_records=num_record_, foldername="./data/",
                measure_number=1, options=options)
            ...
            plot_update_queue.put((
                "update_record", ax, dashlines, rx_values1_copol,
                rx_values1_crosspol, True, '13GHz'))
            ...
            plot_update_queue.put((
                "update_record", ax, dashlines, rx_values1_copol,
                rx_values1_crosspol, True, '17GHz'))
